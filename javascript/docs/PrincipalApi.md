...
<a name="getDbAccessCredentials"></a>
# **getDbAccessCredentials**
> String getDbAccessCredentials()
...
let apiInstance = new OpenLatticeApi.PrincipalApi();
apiInstance.getDbAccessCredentials((error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
...
<a name="searchAllUsersByEmail"></a>
