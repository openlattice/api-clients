  ...
  public = list(
    ...
    initialize = function(`sql`=NULL, `src`=NULL, `srcPrimaryKeyColumns`=NULL, `path`=NULL, `flight`=NULL, ...){
      ...
      if (!is.null(`srcPrimaryKeyColumns`)) {
        stopifnot(is.vector(`srcPrimaryKeyColumns`), length(`srcPrimaryKeyColumns`) != 0)
    ...
    fromJSON = function(FlightPlanParametersJson) {
      ...
      if (!is.null(FlightPlanParametersObject$`src`)) {
        self$`src` <- ApiClient$new()$deserializeObj(FlightPlanParametersObject$`src`, "map(character)", loadNamespace("openlattice"))
      }
