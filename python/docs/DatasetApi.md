...
table_id = 'table_id_example' # str | 
    ...
    try:
        # Gets an OrganizationExternalDatabaseTable object, which represents an organization's table in an external database.
        api_response = api_instance.get_external_database_table(organization_id, table_id)
        pprint(api_response)
    except ApiException as e:
...
> dict(str, object) get_external_database_table_data(organization_id, table_id, row_count)
...
row_count = 56 # int | 
    ...
    try:
        # Gets an OrganizationExternalDatabaseTable object with user specified number of rows of raw data for an organization
        api_response = api_instance.get_external_database_table_data(organization_id, table_id, row_count)
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling DatasetApi->get_external_database_table_data: %s\n" % e)
```
